...
The WRAMPsys board provides a number of I/O devices. This document
...
All WRAMPsys I/O devices are memory mapped. This means that to access a
...
The WRAMPsys board provides two independent RS232 serial interfaces.
...
This port is used by the monitor software on the WRAMPsys
board to communicate with the user and to allow software to be
uploaded to the WRAMPsys board. The second serial port is attached to a
Digital VT320 dumb serial terminal via the Pmod peripherals mirco-USB.
...
The serial ports provided on the WRAMPsys board can operate in either
...
The WRAMPsys board monitor will initialise the serial port so that no
...
The parallel interface on the WRAMPsys board provides an input interface
...
The Programmable Timer on the WRAMPsys board provides for generation of
