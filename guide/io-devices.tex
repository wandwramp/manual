% pdfLaTeX doesn't seem to support fonts in SVG images, so it won't pick up that
% our diagrams have text in them at a particular size in the Courier typeface.
% Let's declare a neat custom command to enforce the font settings we want
% when including SVG files.
\newcommand{\includediagram}[2]{{\fontfamily{pcr}\selectfont\scriptsize \includesvg[width=#1]{#2}}}
...
\begin{figure}[h]
\begin{center}
\includediagram{0.8\textwidth}{switch_reg}
...
\begin{figure}[h]
\begin{center}
%\missingfigure[width=0.8\textwidth]{LED.eps}
...
% Undefine our neat custom command so it doesn't mess up anything unexpected in future
\let\includediagram\undefined
