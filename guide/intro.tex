...
\todo[color=blue!30]{Bundle intro.s}
...
memory. This may be used to move execution to a different area of code or to refer to data in the memory.
%
...
In the example program, we have declared the `entry point' \src{main} as global. To do this we used the following directive:
\begin{verbatim}
   .global main
\end{verbatim}
...
Stores the address of the next instruction into the return address register \reg{ra}. It then jumps to the specified label. This is used when calling subroutines.
...
Before we can execute our program on the Basys board we need to translate it into machine code, so that the WRAMP CPU can understand it. This is a two-step process.
...
\todo[color=blue!30]{Bundle lib\_manual.o}
Issue the command:
\begin{verbatim}
...
board by pressing the \src{RESET} button, the \src{RESET} button is the top button in the group of 5. 
...
\src{main} label. The program will prompt you to enter four numbers. You should continue to type a number and hit enter until the program outputs the resulting
...
The program that you have just run on the Basys board should have read four numbers from the keyboard and output their sum to the terminal. However, when
